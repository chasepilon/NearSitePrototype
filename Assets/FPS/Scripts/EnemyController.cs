...
public class EnemyController : MonoBehaviour
{
    ...
    public List<PatrolPath> patrolPath = new List<PatrolPath>();
    public float patrolChangeMinDelay = 1f;
    public float patrolChangeMaxDelay = 10f;
    ...
    void Update()
    {
        ...
        if (m_EnemyMobile.isPatrolling())
        {
            m_teleportTimer += Time.deltaTime;

            if (m_teleportTimer > currentDelay)
            {
                ChangePatrolPath();
            }
        }
    ...
    {
        m_teleportTimer = 0f;
        int newIndex = Random.Range(0, patrolPath.Count);
        // So we don't get the same index...
        while(newIndex == currentPatrolPath)
            newIndex = Random.Range(0, patrolPath.Count);

        currentPatrolPath = newIndex;
        currentDelay = Random.Range(patrolChangeMinDelay, patrolChangeMaxDelay);
        transform.position = patrolPath[currentPatrolPath].pathNodes[0].transform.position;
        teleportSound.Play();
    }
    ...
    {
        return patrolPath[currentPatrolPath] && patrolPath[currentPatrolPath].pathNodes.Count > 0;
    }
    ...
    public void SetPathDestinationToClosestNode()
    {
        if (IsPathValid())
        {
            ...
            for (int i = 0; i < patrolPath[currentPatrolPath].pathNodes.Count; i++)
            {
                float distanceToPathNode = patrolPath[currentPatrolPath].GetDistanceToNode(transform.position, i);
                if (distanceToPathNode < patrolPath[currentPatrolPath].GetDistanceToNode(transform.position, closestPathNodeIndex))
                {
    ...
    public Vector3 GetDestinationOnPath()
    {
        if (IsPathValid())
        {
            return patrolPath[currentPatrolPath].GetPositionOfPathNode(m_PathDestinationNodeIndex);
        }
        else
        {
    ...
    public void UpdatePathDestination(bool inverseOrder = false)
    {
        if (IsPathValid())
        {
            ...
            if ((transform.position - GetDestinationOnPath()).magnitude <= pathReachingRadius)
            {
                ...
                {
                    m_PathDestinationNodeIndex += patrolPath[currentPatrolPath].pathNodes.Count;
                }
                if (m_PathDestinationNodeIndex >= patrolPath[currentPatrolPath].pathNodes.Count)
                {
                    m_PathDestinationNodeIndex -= patrolPath[currentPatrolPath].pathNodes.Count;
                }
            }
